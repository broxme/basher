...
#
#
# Basher script to create or backup a BasH Linux ISO
# 
# Modified to basher from remastersys.
#
# For use on BasH Linux .
# Copyright (C) 2019 
# BroxMe Technology 
# BroxMe Developer Network
...
#Modified to 'basher' by: Stace Fauske <stace.fauske@gmail.com> <-sef> - (C) 2015
# 
# This program is distributed in the hope that it will be useful,
# but WITHOUT ANY WARRANTY; without even the implied warranty of
# MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
...
class appgui:
    ...
    def on_button1_clicked(self,widget):
        ...
        if not msg_confirm(_("You have selected Backup Mode. Do not interrupt this process. Click OK to Start the Backup LiveCD/DVD process."), self.window1.get_widget("window1")):
            return
        self.run_command('basher backup', self.on_backup_done)
    ...
    def on_backup_done(self, widget, data = None):
        if self.v.get_child_exit_status() == 0:
            ...
            msg_info(_("Your %(iso)s and %(iso)s.md5 files are ready in %(dir)s. It is recommended to test it in a virtual machine or on a rewritable cd/dvd to ensure it works as desired. Click on OK to return to the main menu.") 
                % ({"iso" : CUSTOMISO, "dir" : WORKDIR+'/basher'}), self.window1.get_widget("window1"))
        else:
    ...
    def on_button2_clicked(self,widget):
        ...
        if not msg_confirm(_("You have selected Dist Mode. Click OK to Start the Distributable LiveCD/DVD process."), self.window1.get_widget("window1")):
            return
        self.run_command('basher dist', self.on_dist_done)
    ...
    def on_dist_done(self, widget, data = None):
        ...
        if self.v.get_child_exit_status() == 0:
            WORKDIR = self.window1.get_widget("entry6").get_text()
            CUSTOMISO = self.window1.get_widget("entry3").get_text()
            msg_info(_("Your %(iso)s and %(iso)s.md5 files are ready in %(dir)s. It is recommended to test it in a virtual machine or on a rewritable cd/dvd to ensure it works as desired. Click on OK to return to the main menu.") % ({"iso" : CUSTOMISO, "dir" : WORKDIR+'/basher'}))
        else:
            msg_error(_("The process was interrupted!"), self.window1.get_widget("window1"))
        self.window1.get_widget("notebook1").set_current_page(0)
        self.v.handler_disconnect(self.callback_id)
    ...
    def on_button3_clicked(self,widget):
        ...
        if not msg_confirm(_("You have selected Dist CDFS Mode. Click OK to Start the Distributable LiveCD/DVD filesystem build process.")):
            return
        self.run_command('basher dist cdfs', self.on_dist_cdfs_done)

    def on_dist_cdfs_done(self, widget, data = None):
        if self.v.get_child_exit_status() == 0:
            WORKDIR = self.window1.get_widget("entry6").get_text()
            CUSTOMISO = self.window1.get_widget("entry3").get_text()
            msg_info(_("Your livecd filesystem is ready in %s. You can now add files to the cd and then run the Distiso option when you are done. Click on OK to return to the main menu.") % WORKDIR+'/basher')
        else:
            msg_error(_("The process was interrupted!"), self.window1.get_widget("window1"))
        self.window1.get_widget("notebook1").set_current_page(0)
        self.v.handler_disconnect(self.callback_id)
    ...
    def on_button4_clicked(self,widget):
        self.update_conf()
        WORKDIR = self.window1.get_widget("entry6").get_text()
        if os.path.exists(WORKDIR+'/basher/ISOTMP/casper/filesystem.squashfs'):
            if not msg_confirm(_("You have selected Dist ISO Mode. Click OK to create the iso file.")):
                self.window1.get_widget("window1").show()
                return
            self.run_command('basher dist iso', self.on_dist_iso_done)
        else:
    ...
    def on_button5_clicked(self,widget):
        ...
        if not msg_confirm(_("This will remove all the files from the temporary directory. Click OK to proceed.")):
            return
        #os.system('basher clean')
        self.run_command('basher clean', self.on_clean_done)
    ...
    def on_button6_clicked(self,widget):
        ...
        about.set_program_name(_("Basher"))
        about.set_version(APP_VERSION)
        about.set_authors([_(" "),
            _("       -- basher --"),
            _("BroxMe Technology<support@broxme.com>"),
            _("BroxMe Developer Network <developer@broxme.com>"),
            _("Fahim Islam Sabuj<fahim@broxme.com>"),
            _(" "),
            _("       -- remastersys --"),
            _("Krasimir S. Stefanov <lokiisyourmaster@gmail.com>"),
            _("Tony Brijeski <tb6517@yahoo.com>")
        ])
        about.set_website("http://bash.broxme.com/")
        translators = [
        ...
        about.set_logo_icon_name('basher-gtk')
        license = _('''PyGTK GUI for Basher
Copyright (C) 2011 Krasimir S. Stefanov, Tony Brijeski
Modified to BroxMe Technology - (C) 2019
This is Free Software
Originally Remastersys, modified to Basher
...
http://www.gnu.org/licenses/.''')
    ...
    def on_button9_clicked(self,widget):
        ...
        if response == gtk.RESPONSE_OK:
            ...
            shutil.move("/etc/basher/isolinux/splash.png", "/etc/basher/isolinux/splash.png." + now.strftime("%Y%m%d%H%M%S"))
            shutil.copy(filename, "/etc/basher/isolinux/splash.png")
            msg_info(_("%s has been copied to /etc/basher/isolinux/splash.png becoming the default background for the LIVE menu.") % filename)
        else:
            dialog.destroy()                  
    ...
    def on_button11_clicked(self,widget):
        ...
        def ok(widget, other = None):
            ...
            process = subprocess.Popen(['basher-skelcopy', username], stdout=subprocess.PIPE, stderr=None)
            while process.poll() == None:
                while gtk.events_pending():
                    gtk.main_iteration_do()
                time.sleep(.1) 
    ...
    def on_button12_clicked(self,widget):
        ...
        def create(widget, other = None):
            theme_name = msg_input('', _('Enter your plymouth theme name. eg. Basher Theme (please use only alphanumeric characters)'), _('Name:'), 'Basher Theme')          
            ...
            if response == gtk.RESPONSE_OK:
                ...
                shutil.copy('/etc/basher/plymouth/basher-theme/progress_bar.png', theme_dir+'/progress_bar.png')
                shutil.copy('/etc/basher/plymouth/basher-theme/progress_box.png', theme_dir+'/progress_box.png')
                script_name = "/lib/plymouth/themes/"+theme_name_fixed+"/"+theme_name_fixed+".script"
                script = open("/etc/basher/plymouth/basher-theme/basher-theme.script").read().replace("__THEMEPIC__", os.path.basename(theme_pic))
                open(script_name, 'w+').write(script)
                
                config_name = "/lib/plymouth/themes/"+theme_name_fixed+"/"+theme_name_fixed+".plymouth"
                config = open("/etc/basher/plymouth/basher-theme/basher-theme.plymouth").read()
    ...
    def load_settings(self):
        ...
        self.window1.get_widget("entry5").set_text(
            self.getvalue('LIVECDURL', config_txt, 'http://bash.broxme.com'))
        ...
        workdir = self.getvalue('WORKDIR', config_txt, '/home/basher')
        ...
        self.window1.get_widget("entry10").set_text(
            self.getvalue('DISTNAME', config_txt, 'BasH Linux'))
    ...
    def update_conf(self):
        ...
        conf_content = '''
#Basher Global Configuration File
...
# Example:
#~ DISTICONS="AwOken-BasH hicolor myfavoriteicon1.png myfavoriteicon2.png"
...
if os.popen('whoami').read().strip() != 'root':
    ...
    if os.system('which gksu')==0:
        if os.path.exists('/usr/share/applications/basher-gtk.desktop'):
            os.system('gksu -D "%s" ./basher-gtk.py' % _('Basher'))
        else:
            os.system('gksu -D "%s" ./basher-gtk.py' % '/usr/share/applications/basher-gtk.desktop')
    elif os.system('which kdesudo')==0:
        os.system('kdesudo ./basher-gtk.py' )
    elif os.system('which sudo')==0:
        password = msg_input(_(''), _('Enter your password to perform administrative tasks'), 'Password:', '', None, True)
        if password:
            os.popen('sudo -S ./basher-gtk.py','w').write(password)
else:
